...
def procesar_datos(df):
    ...
    # Diccionario de meses en español (sin necesidad de locale)
    meses_es = {
        1: "enero",
        2: "febrero",
        3: "marzo",
    ...
    df["Año"] = df["Fecha inicio"].dt.year
    df["Mes"] = df["Fecha inicio"].dt.month.map(meses_es)
    df["Día"] = df["Fecha inicio"].dt.day
...
if archivo is not None:
    ...
    st.stop()   # 👈 DETIENE LA EJECUCIÓN PARA EVITAR NameError
...
with col2:
    mes_sel = st.multiselect(
        "Selecciona mes(es):",
        list(df["Mes"].dropna().unique()),
        default=list(df["Mes"].dropna().unique())
    )

with col3:
    dia_sel = st.multiselect(
        "Selecciona día(s):",
        sorted(df["Día"].dropna().unique()),
        default=sorted(df["Día"].dropna().unique())
    )

with col4:
    mesa_sel = st.selectbox(
        "Selecciona una mesa:",
        df["Mesa original"].dropna().unique()
    )
